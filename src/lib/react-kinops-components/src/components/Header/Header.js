...
const BuildKappLink = ({ kapp, nameOverride = kapp.name }) =>
  <DropdownItem tag="a" href={bundle.kappLocation(kapp.slug)}>
    <span
      className={classNames('fa fa-fw', getAttributeValue(kapp, 'Icon') || 'fa-book')}
    />{nameOverride}
  </DropdownItem>;

export const Header = ({
  hasSidebar, toggleSidebarOpen,
      ...
      { !isGuest &&
        <UncontrolledDropdown>
          ...
          <DropdownMenu>
            ...
            { (hasAccessToManagement || hasAccessToSupport) && <DropdownItem divider /> }
            { hasAccessToManagement && <BuildKappLink kapp={adminKapp} nameOverride="Admin Console" />}
            { hasAccessToSupport &&
              <DropdownItem tag="a" href={`${bundle.kappLocation(adminKapp.slug)}/submission-support`}>
